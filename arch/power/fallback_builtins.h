 ...
 * xxpermdi instruction that only moves the registers __a, __b instead of
 * generating a load.
 *
 * Clang has vec_xxpermdi intrinsics. It was implemented in 4.0.0.
 */
static inline __vector unsigned long long __builtin_pack_vector(unsigned long __a, unsigned long __b) {
 ...
 * behavior as GCC. That means code adapted for Clang >= 7 does not work on
 * Clang <= 6.  So, fallback to __builtin_unpack_vector() on Clang <= 6.
 */
#if !defined vec_xxpermdi || __clang_major__ <= 6

static inline unsigned long __builtin_unpack_vector(__vector unsigned long long __v, int __o) {
    return __v[__o];
}
...
static inline unsigned long __builtin_unpack_vector_0(__vector unsigned long long __v) {
#if BYTE_ORDER == BIG_ENDIAN
...
static inline unsigned long __builtin_unpack_vector_1(__vector unsigned long long __v) {
#if BYTE_ORDER == BIG_ENDIAN
    return vec_xxpermdi(__v, __v, 0x3)[0];
#else
    return vec_xxpermdi(__v, __v, 0x0)[0];
