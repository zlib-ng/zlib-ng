...
#define d_code(dist) ((dist) < 256 ? PREFIX4(dist_code)[dist] : PREFIX4(dist_code)[256+((dist)>>7)])
...
# if defined(GEN_TREES_H)
    extern unsigned char ZLIB_INTERNAL PREFIX4(length_code)[];
    extern unsigned char ZLIB_INTERNAL PREFIX4(dist_code)[];
# else
    extern const unsigned char ZLIB_INTERNAL PREFIX4(length_code)[];
    extern const unsigned char ZLIB_INTERNAL PREFIX4(dist_code)[];
# endif
...
# define _zng_tr_tally_dist(s, distance, length, flush) \
  { unsigned char len = (unsigned char)(length); \
    ...
    dist--; \
    s->dyn_ltree[PREFIX4(length_code)[len]+LITERALS+1].Freq++; \
    s->dyn_dtree[d_code(dist)].Freq++; \
    flush = (s->sym_next == s->sym_end); \
  }
#else
# define _zng_tr_tally_lit(s, c, flush) flush = PREFIX4(tr_tally)(s, 0, c)
# define _zng_tr_tally_dist(s, distance, length, flush) \
            flush = PREFIX4(tr_tally)(s, (unsigned)(distance), (unsigned)(length))
#endif

#if defined(ZLIB_COMPAT)
# define _tr_tally_lit  _zng_tr_tally_lit
# define _tr_tally_dist _zng_tr_tally_dist
#else
# define zng_tr_tally_lit  _zng_tr_tally_lit
# define zng_tr_tally_dist _zng_tr_tally_dist
#endif
