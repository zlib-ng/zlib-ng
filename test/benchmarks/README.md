...
| `--benchmark_no_aslr` | macOS, Linux | Re-exec in place with ASLR disabled so code and stack addresses are identical every launch, removing run-to-run layout luck. Preserves the pid, so a parent waiting on the process is unaffected. |
