/* test_deflate_params.c - Test deflate() with dynamic change of compression level */
...
int main() {
    ...
    if (d_stream.total_out != (2 * uncompr_len) + diff)
        error("bad params inflate: %" PRIu64 "\n", (uint64_t)d_stream.total_out);
    else
        printf("deflateparams(): OK\n");
