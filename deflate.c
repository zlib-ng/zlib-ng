...
Z_INTERNAL const char PREFIX(deflate_copyright)[] = " deflate 1.3.1 Copyright 1995-2024 Jean-loup Gailly and Mark Adler ";
/*
...
Z_INTERNAL deflate_allocs* alloc_deflate(PREFIX3(stream) *strm, int windowBits, int lit_bufsize) {
    ...
    /* Define sizes */
    int window_size = DEFLATE_ADJUST_WINDOW_SIZE(W_BUF_SIZE(1 << windowBits));
...
int32_t Z_EXPORT PREFIX(deflateCopy)(PREFIX3(stream) *dest, PREFIX3(stream) *source) {
    ...
    memcpy(ds->window, ss->window, DEFLATE_ADJUST_WINDOW_SIZE(W_BUF_SIZE(ds->w_size) * sizeof(unsigned char)));
...
void Z_INTERNAL PREFIX(fill_window)(deflate_state *s) {
    ...
    do {
        ...
        if (s->fill_end > s->window_size - MIN_LOOKAHEAD) {
            ...
            memcpy(window, window + slide_len, s->strstart + s->lookahead - slide_len);
            if (s->match_start >= slide_len) {
                s->match_start -= slide_len;
            } else {
                s->match_start = 0;
                s->prev_length = 0;
            }
