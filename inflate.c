...
int ZEXPORT PREFIX(inflateResetKeep)(PREFIX3(stream) *strm) {
    ...
    state->mode = HEAD;
    state->check = adler32_impl(0L, NULL, 0);
    state->last = 0;
    state->havedict = 0;
    state->dmax = 32768U;
    state->head = NULL;
...
#  define UPDATE(check, buf, len) \
    (state->flags ? PREFIX(crc32)(check, buf, len) : adler32_impl(check, buf, len))
#else
#  define UPDATE(check, buf, len) adler32_impl(check, buf, len)
#endif
...
int ZEXPORT PREFIX(inflate)(PREFIX3(stream) *strm, int flush) {
    ...
    for (;;)
        ...
        case HEAD:
            ...
            strm->adler = state->check = adler32_impl(0L, NULL, 0);
...
int ZEXPORT PREFIX(inflateSetDictionary)(PREFIX3(stream) *strm, const unsigned char *dictionary, unsigned int dictLength) {
    ...
    if (state->mode == DICT) {
        dictid = adler32_impl(0L, NULL, 0);
        dictid = adler32_impl(dictid, dictionary, dictLength);
